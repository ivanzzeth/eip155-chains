[**eip155-chains**](README.md) • **Docs**

***

# eip155-chains

## Interfaces

- [ApiKeys](interfaces/ApiKeys.md)
- [ClassifiedRpc](interfaces/ClassifiedRpc.md)
- [EIP155Chain](interfaces/EIP155Chain.md)
- [Filters](interfaces/Filters.md)
- [Options](interfaces/Options.md)
- [RPC](interfaces/RPC.md)
- [RpcService](interfaces/RpcService.md)

## Type Aliases

- [RpcFeature](type-aliases/RpcFeature.md)
- [RpcFeatureFree](type-aliases/RpcFeatureFree.md)
- [RpcFeatureLoadBalance](type-aliases/RpcFeatureLoadBalance.md)
- [RpcFeatureMevProtection](type-aliases/RpcFeatureMevProtection.md)
- [RpcFeaturePhishingProtection](type-aliases/RpcFeaturePhishingProtection.md)
- [RpcFeaturePrivacy](type-aliases/RpcFeaturePrivacy.md)
- [RpcFeatureSmartContractMethodChecks](type-aliases/RpcFeatureSmartContractMethodChecks.md)
- [RpcFeatureSmartContractWhitelist](type-aliases/RpcFeatureSmartContractWhitelist.md)
- [RpcList](type-aliases/RpcList.md)
- [RpcUrl](type-aliases/RpcUrl.md)

## Variables

- [ChainNotFound](variables/ChainNotFound.md)
- [KeyNotFound](variables/KeyNotFound.md)
- [RPC\_TIMEOUT](variables/RPC_TIMEOUT.md)
- [RpcNotFound](variables/RpcNotFound.md)
- [ViemChainInfoNotFound](variables/ViemChainInfoNotFound.md)
- [registeredRpcServices](variables/registeredRpcServices.md)

## Functions

- [buf2hex](functions/buf2hex.md)
- [filterRpcListByFeatures](functions/filterRpcListByFeatures.md)
- [fulfillRpcFeatures](functions/fulfillRpcFeatures.md)
- [getChainById](functions/getChainById.md)
- [getChainMetadataById](functions/getChainMetadataById.md)
- [getChainMetadataByIdByChainIdNetwork](functions/getChainMetadataByIdByChainIdNetwork.md)
- [getChainMetadataByIdByEthChains](functions/getChainMetadataByIdByEthChains.md)
- [getChainMetadataByIdByLocalFile](functions/getChainMetadataByIdByLocalFile.md)
- [getRandomString](functions/getRandomString.md)
- [getRandomUint8Array](functions/getRandomUint8Array.md)
- [getRpcsByChainId](functions/getRpcsByChainId.md)
- [getViemChainInfo](functions/getViemChainInfo.md)
- [healthyCheck](functions/healthyCheck.md)
- [isValidRpcProtocol](functions/isValidRpcProtocol.md)
- [registerRpcService](functions/registerRpcService.md)
- [sleep](functions/sleep.md)
- [splitRpcsByProtocol](functions/splitRpcsByProtocol.md)
